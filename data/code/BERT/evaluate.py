...
def parse_args():
    parser = argparse.ArgumentParser(
        'Official evaluation script for KAPT version 1.0.')
    parser.add_argument('--data_file', metavar='KCT_train_public.txt',
                        help='Input dataset file.')  # 数据文件，格式同训练集
    parser.add_argument('--pred_file', metavar='pred_answer.csv',
                        help='Model predictions.')  # CSV预测结果文件
...
def f1_score(df):
    ...
    for item in df.iterrows():
        ...
        f1_scores.append(f1_scores_item)
...
def judge(standardResultFile, userCommitFile):
    ...
    try:
        ...
        # 检查id值域应一致
        cd_flag = len(
            set(np.unique(label_df['qid'])) - set(np.unique(pred_df['id'])))

        chk_df = pd.merge(label_df, pred_df, left_on='qid',
                          right_on='id', suffixes=('_label', '_pred'))
